...
NETWORK=$MLKC_CONTEXT_DATA_NETWORK
# Container name.
CONTAINER_NAME=$MLKC_CONTEXT_DATA_APP_NAME_process
# Container host name.
CONTAINER_HOST_NAME=$MLKC_CONTEXT_DATA_APP_NAME_process
...
if [ ! -z "$REPLICAS" ] ; then
  ...
  eval  $COMMAND \
        -e MLKC_CONTEXT_DATA_HOST=${MLKC_CONTEXT_DATA_HOST} \
        -e MLKC_CONTEXT_DATA_POSTGIS_PASSWORD=${MLKC_CONTEXT_DATA_POSTGIS_PASSWORD} \
        -e MLKC_CONTEXT_DATA_PG_EXTERNAL_PORT=${MLKC_CONTEXT_DATA_PG_EXTERNAL_PORT} \
        $NETWORK \
        ${CONTAINER_NAME} \
        ${CONTAINER_HOST_NAME} \
        $X11 \
